...
class DetodoyaMessages {
    ...
    showMessage(message, type = 'info', duration = 5000) {
        ...
        messageDiv.className = `detodoya-message detodoya-message-${type}`;
        ...
        messageDiv.innerHTML = `
            <div class="detodoya-message-content">
                <div class="detodoya-message-header">
                    <span class="detodoya-message-title">Detodoya dice:</span>
                </div>
...
function showOriolaMessage(type, message, duration) {
    switch(type) {
        case 'error':
        case 'danger':
            detodoyaMessages.showError(message, duration);
            break;
        case 'success':
            detodoyaMessages.showSuccess(message, duration);
            break;
        case 'info':
            detodoyaMessages.showInfo(message, duration);
            break;
        case 'warning':
            detodoyaMessages.showWarning(message, duration);
            break;
        default:
            detodoyaMessages.showInfo(message, duration);
    }
...
window.alert = function(message) {
    ...
    if (message.includes('❌') || message.toLowerCase().includes('error')) {
        detodoyaMessages.showError(message.replace('❌', '').trim());
    } else if (message.includes('✅') || message.toLowerCase().includes('éxito') || message.toLowerCase().includes('exitoso')) {
        detodoyaMessages.showSuccess(message.replace('✅', '').trim());
    } else {
        detodoyaMessages.showInfo(message);
    }
};

// Reemplazar confirm() nativo con el sistema Detodoya
const originalConfirm = window.confirm;
window.confirm = async function(message) {
    return await detodoyaMessages.confirm(message);
};
